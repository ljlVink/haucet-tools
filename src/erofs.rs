...
mod tests {
    ...
    #[test]
    fn parses_and_remaps_recorded_options() {
        let temp = tempfile::tempdir().unwrap();
        let config = temp.path().join("config");
        fs::create_dir(&config).unwrap();
        fs::write(config.join("cust_fs_config"), "").unwrap();
        fs::write(config.join("cust_file_contexts"), "").unwrap();
        let options = config.join("cust_fs_options");
        fs::write(
            &options,
            "mkfs.erofs options: -zlz4hc,1 -T 123 -U abc --mount-point=/cust \
             --fs-config-file=/old/cust_fs_config --file-contexts=/old/cust_file_contexts \
             old.img old-root\n",
        )
        .unwrap();
        let parsed = parse_mkfs_options(&options, &config).unwrap();
        assert!(parsed.contains(&"-zlz4hc,1".to_owned()));
        assert!(parsed.contains(&format!(
            "--fs-config-file={}",
            config.join("cust_fs_config").display()
        )));
        assert!(!parsed.iter().any(|word| word == "old.img"));
    }

    #[test]
    fn rejects_output_shaping_options() {
        let temp = tempfile::tempdir().unwrap();
        let options = temp.path().join("fs_options");
        fs::write(
            &options,
            "mkfs.erofs options: --blobdev=/tmp/other old.img root\n",
        )
        .unwrap();
