...
mod tests {
    ...
    fn package(layout: UpdateLayout, entries: &[(&str, u8, &[u8])]) -> Vec<u8> {
        let record_len = layout.record_len();
        let address_len = layout.address_len();
        let mut bytes = vec![0_u8; HEADER_LEN];
        bytes[0..2].copy_from_slice(
            &(if layout == UpdateLayout::L2 {
                0x01_u16
            } else {
                0x11_u16
            })
        ...
        let table_len = record_len * entries.len();
        bytes[COMPINFO_LEN_OFFSET..HEADER_LEN].copy_from_slice(&(table_len as u16).to_le_bytes());
        for (name, kind, payload) in entries {
            let mut record = vec![0_u8; record_len];
            record[..name.len()].copy_from_slice(name.as_bytes());
            record[address_len + 4] = *kind;
            record[address_len + 15..address_len + 23]
                .copy_from_slice(&(payload.len() as u64).to_le_bytes());
            bytes.extend(record);
        }
        bytes.extend([0_u8; RESERVE_LEN]);
    ...
    #[test]
    fn parses_l2_and_large_size_field() {
        let bytes = package(UpdateLayout::L2, &[("system", 0, b"erofs")]);
        let mut cursor = Cursor::new(bytes.clone());
        let index = read_index(&mut cursor, Some(bytes.len() as u64), UpdateLayout::Auto).unwrap();
        assert_eq!(index.layout, UpdateLayout::L2);
        assert_eq!(index.components[0].output_name, "system.img");
        assert_eq!(index.components[0].size, 5);
    }

    #[test]
    fn streams_components_to_files() {
        let bytes = package(
            UpdateLayout::L1,
            &[("version_list", 2, b"version"), ("vendor", 0, b"image")],
        );
        let output = tempfile::tempdir().unwrap();
        let components = unpack_reader(
        ...
        assert_eq!(
            fs::read(output.path().join("VERSION.mbn")).unwrap(),
            b"version"
        );
        assert_eq!(
            fs::read(output.path().join("vendor.img")).unwrap(),
            b"image"
        );
    }
    ...
    fn rejects_truncated_payload() {
        let mut bytes = package(UpdateLayout::L2, &[("system", 0, b"erofs")]);
        bytes.pop();
        let mut cursor = Cursor::new(bytes.clone());
        let error = read_index(&mut cursor, Some(bytes.len() as u64), UpdateLayout::Auto)
            .unwrap_err()
            .to_string();
    ...
    fn rejects_unsafe_name() {
        let bytes = package(UpdateLayout::L2, &[("../system", 0, b"erofs")]);
        let mut cursor = Cursor::new(bytes.clone());
        assert!(read_index(&mut cursor, Some(bytes.len() as u64), UpdateLayout::Auto).is_err());
    }
}
